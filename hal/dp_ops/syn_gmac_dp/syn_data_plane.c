...
static int syn_dp_napi_poll(struct napi_struct *napi, int budget)
{
	...
	dev_info->stats.stats.rx_missed += syn_get_rx_missed(gmac_dev->gmac_hal_ctx);
	dev_info->stats.stats.rx_missed += syn_get_fifo_overflows(gmac_dev->gmac_hal_ctx);
...
/*
 * syn_dp_if_get_ndo_stats
 *	Get Synopsys GMAC data plane stats
 */
static void syn_dp_if_get_ndo_stats(struct nss_dp_data_plane_ctx *dpc,
...
struct nss_dp_data_plane_ops nss_dp_gmac_ops = {
	...
	.get_stats	= syn_dp_if_get_ndo_stats,
